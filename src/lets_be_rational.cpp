...
#define SQRT_THREE_OVER_THIRD_ROOT_TWO_PI 0.938643487427383566075051356115075878414688769574 // sqrt3 / sqrt3(2pi)
...
#else
#define IS_REGION_I(thetax,s)   (thetax < s * ETA && s * (0.5 * s - (TAU + 0.5 + ETA)) + thetax < 0) // h < eta,  t < (tau+12) + (|h|-|eta|)  equiv  s/2 < (tau+12) - theta*x/s + eta  equiv  s*(s/2-(tau+12+eta))+x < 0     using    h = theta*x/s and t = s/2
#define IS_REGION_II(thetax,s)  (s * (s - (2 * TAU)) - thetax / ETA < 0) // t < tau + (12/|eta|)*|h|  equiv  s < 2*tau + (theta*x/s)/eta  equiv  s*(s-2*tau)-theta*x/eta < 0                  using    h = theta*x/s and t = s/2
#endif
...
#define IS_REGION_III(thetax,s) (s * (0.5 * s - 0.85) + thetax > 0) // t+h > 0.85  equiv  s/2+theta*x/s > 0.85  equiv  s*(s/2-0.85)+x > 0      using    h = theta*x/s and t = s/2
  ...
  static atomic_t implied_volatility_maximum_iterations = { 2 }; // (DBL_DIG*20)/3 simeq 100 . Only needed when the iteration effectively alternates Householder/Halley/Newton steps and binary nesting due to roundoff truncation.
...
#else
  const double a = Yprime(h), hsuperscripttwo = h * h, tsuperscripttwo = t * t;
# define B0 2*a
# define B1 (-1+a*(3+hsuperscripttwo))/3
# define B2 (-7-hsuperscripttwo+a*(15+hsuperscripttwo*(10+hsuperscripttwo)))/60
# define B3 (-57+(-18-hsuperscripttwo)*hsuperscripttwo+a*(105+hsuperscripttwo*(105+hsuperscripttwo*(21+hsuperscripttwo))))/2520
# define B4 (-561+hsuperscripttwo*(-285+(-33-hsuperscripttwo)*hsuperscripttwo)+a*(945+hsuperscripttwo*(1260+hsuperscripttwo*(378+hsuperscripttwo*(36+hsuperscripttwo)))))/181440
# define B5 (-6555+hsuperscripttwo*(-4680+hsuperscripttwo*(-840+(-52-hsuperscripttwo)*hsuperscripttwo))+a*(10395+hsuperscripttwo*(17325+hsuperscripttwo*(6930+hsuperscripttwo*(990+hsuperscripttwo*(55+hsuperscripttwo))))))/19958400
# define B6 (-89055+hsuperscripttwo*(-82845+hsuperscripttwo*(-20370+hsuperscripttwo*(-1926+(-75-hsuperscripttwo)*hsuperscripttwo)))+a*(135135+hsuperscripttwo*(270270+hsuperscripttwo*(135135+hsuperscripttwo*(25740+hsuperscripttwo*(2145+hsuperscripttwo*(78+hsuperscripttwo)))))))/3113510400
  return t * (B0 + tsuperscripttwo * (B1 + tsuperscripttwo * (B2 + tsuperscripttwo * (B3 + tsuperscripttwo * (B4 + tsuperscripttwo * (B5 + B6 * tsuperscripttwo))))));
#endif
}
...
// ⟹  θ can be subsumed into x (or h)
//
//     b(x,s)  =  ½ · exp(-u²-v²) · [ erfcx(u+v) -  erfcx(u-v) ]
//             =  ½ · [ exp(θx/2)·erfc(u+v)     -  exp(-θx/2)·erfc(u-v)    ]
//             =  ½ · [ exp(θx/2)·erfc(u+v)     -  exp(-u²-v²)·erfcx(u-v) ]
//             =  ½ · [ exp(-u²-v²)·erfcx(u+v) -  exp(-θx/2)·erfc(u-v)    ]
// and
//              u  = -h/√2  and   v  =  -t/√2 .
//
...
double normalised_volga(double x, double s) {
  ...
  const double h = x / s, t = 0.5 * s, hsuperscripttwo = h * h, tsuperscripttwo = t * t;
  return (1 / SQRT_TWO_PI) * exp(-0.5 * (hsuperscripttwo + tsuperscripttwo)) * (hsuperscripttwo - tsuperscripttwo) / s;
}
...
// 
//    b̄(x,s)    =   ½ · [ erfc((s/2+x/s)/√2)·exp(θx/2) + erfc((s/2-x/s)/√2)·exp(-θx/2) ]              (2)          using  Φ(z) = erfc(-z/√2)/2
//
// using erfc(z) = erfcx(z)·exp(-z²)
// 
//    b̄(x,s)    =   ½ · ( erfcx((s/2+x/s)/√2)·exp(-½((x/s)²+x+(s/2)²))·exp(θx/2) + erfcx((s/2-x/s)/√2)·exp(-½((x/s)²-x+(s/2)²))·exp(-θx/2) )
//
...
inline double b_l_over_b_max(const double s_c /* = sqrt|2x| */) {
...
inline double b_u_over_b_max(const double s_c /* = sqrt|2x| */) {
...
double implied_normalised_volatility_atm(double ibeta) {
  const double ibeta_max = 0.6826894921370859; // = 1-erfc(1/sqrt2)
  if (ibeta <= ibeta_max) {
    // Note that this branch domain corresponds exactly to innermost branch of Φ⁻¹() in algorithm "PJ-2024-Inverse-Normal" denoted as 'inverse_norm_cdfm½_for_midrange_probabilities()'
    ...
    //   s = √8 · ( inverse_norm_cdfm½_for_midrange_probabilities(𝛽/2) / √2 ) .
    ...
    //   s = √(2π) · 𝛽
    //     ≈ 2.507 · 𝛽
    // i.e., s > 𝛽 by more than a factor of two. Expanding inverse_norm_cdfm½_for_midrange_probabilities(y) for very small y does indeed give
    //   inverse_norm_cdfm½_for_midrange_probabilities(y) ≈ √(2π) · y
    ...
    const double r = ibeta_max * ibeta_max - ibeta * ibeta;
...
// Specialisation for x = 0 where bₐₜₘ(s) = 1-2·Φ(-s/2) = 2·Φ(s/2)-1 = erf(s/√8).
#define implied_normalised_volatility_atm(ibeta) ((2 * SQRT_TWO) * erfinv(ibeta))
...
//
double lets_be_rational(double ibeta, double thetax, int N) {
  assert(thetax <= 0);
  if (ibeta <= 0)
    return implied_volatility_output(0, 0 == ibeta ? 0 : VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_BELOW_INTRINSIC);
#if defined( POSITIVE_DENORMALISATION_CUTOFF )
  if (ibeta < POSITIVE_DENORMALISATION_CUTOFF) // For positive but denormalised (a.k.a. 'subnormal') prices, we return 0 since it would be impossible to converge to full machine accuracy anyway.
    return implied_volatility_output(0, 0);
#endif
  const double b_max = exp(0.5 * thetax);
  if (ibeta >= b_max)
  ...
  if (0 == thetax) return implied_normalised_volatility_atm(ibeta);
#endif
  int iterations = 0;
  ...
  // Also:
  //  vega(x,s) = ∂b(x,s)/∂s = b'(s) = exp(-½·((x/s)²+(s/2)²) / √(2π)
  // So
  //  vega(x,s_c) = exp(-½·((x/√(2|x|))²+(√(2|x|)/2)²) / √(2π)
  //              = exp(-½·(|x|/2+|x|/2) / √(2π)
  //              = exp(-|x|/2) / √(2π)
  // Note: bₘₐₓ = exp(-|x|/2).
  const double sqrt_ax = sqrt(-thetax), s_c = SQRT_TWO * sqrt_ax, ome = one_minus_erfcx(sqrt_ax), b_c = 0.5 * b_max * ome;
  // Four branches.
  if (ibeta < b_c) {   // LOWER HALF: s < s_c
    ...
    //  s_c  =  √(2|θx|)
    //  b_c  =  b(θx,s_c)  =  exp(θx/2)·Φ(-√|x|/√2+√|x|/√2) - exp(-θx/2)·Φ(-√|x|/√2-√|x|/√2)  =  ½·exp(θx/2) - exp(-θx/2)·Φ(-√(2|x|))
    //  sₗ  :=   s_c - b(x,s_c)/b'(x,s_c)                         =   √(2|x|) - (exp(θx/2)·Φ(0) - exp(-θx/2)·Φ(-√(2|x|)))·exp((|x|/2+|x|/2)/2)·√(2π)
    //      =   √(2|x|) - [ ½ - exp(|x|)·Φ(-√(2|x|)) ] · √(2π)   =   √(2|x|) - √(π/2) · [ 1 - erfcx(√|x|) ]
    // For x ⟶ -∞, we get
    //   sₗ ⟶ √(2|x|) - √(π/2)
    // and for 
    //  bₗ  := b(θx,sₗ)
...
#if defined( USE_UNIVARIATE_RATIONAL_FUNCTION_APPROXIMATIONS_FOR_BL_AND_BU_OVER_BMAX )
    const double b_l = b_l_over_b_max(s_c) * b_max;
#else
    const double b_l = normalised_black(thetax, s_l);
#endif
    if (ibeta < b_l) {   // LOWEST BRANCH:   s < s_l
      {
        ...
        f = rational_cubic_interpolation(ibeta, 0., b_l, 0., f_lower_map_l, 1., d_f_lower_map_l_d_beta, r_ll);
      ...
      //     s_n+1  =  s_n  +  ν · ( 1 + ν·h₂/2 ) / ( 1 + ν·( h₂ + ν·h₃/6 ) ).
      //
      const double ln_ibeta = log(ibeta);
      for (; iterations<N && fabs(ds)>DBL_EPSILON * s; ++iterations) {
        ...
        if (b > ibeta && s < s_right) s_right = s; else if (b<ibeta && s>s_left) s_left = s; // Tighten the bracket if applicable.
...
#endif
        {
          const double h = thetax / s, xsuperscripttwo_over_ssuperscriptthree = h * h / s, b_h_2 = xsuperscripttwo_over_ssuperscriptthree - s / 4, nu = (ln_ibeta - ln_b) * ln_b / ln_ibeta / bpob, lambda = 1 / ln_b, otlambda = 1 + 2 * lambda, h_2 = b_h_2 - bpob * otlambda, c = 3 * (xsuperscripttwo_over_ssuperscriptthree / s) /* = (h/s)superscripttwo */;
      ...
      const double inv_v_c = SQRT_TWO_PI / b_max;  // v_c = b_max * (1 / SQRT_TWO_PI)
      const double inv_v_l = inv_normalised_vega(thetax, s_l), r_lm = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(b_l, b_c, s_l, s_c, inv_v_l, inv_v_c, 0.0, false);
      s = rational_cubic_interpolation(ibeta, b_l, b_c, s_l, s_c, inv_v_l, inv_v_c, r_lm);
      assert(s > 0); // Even without ATM (x=0) specialisation, we are guaranteed to have a positive number here. This assertion is merely a reminder and a safeguard against development errors.
#if defined (ENABLE_CHANGING_THE_MAXIMUM_ITERATION_COUNT) || defined( SAFEGUARD_STEP_AGAINST_BRACKETS )
      s_left = s_l;
      s_right = s_c;
    ...
    const double inv_v_c = SQRT_TWO_PI / b_max;  // v_c = b_max * (1 / SQRT_TWO_PI)
    const double s_u = v_c > DBL_MIN ? s_c + (b_max - b_c) * inv_v_c : s_c;
#else
    const double s_u = s_c + SQRT_PI_OVER_TWO * (2 - ome); // = s_c + (b_max - b_c) / v_c    ---     ome  =  1 - erfcx(sqrt|x|)
#endif
    assert(s_u > 0);
#if defined( USE_UNIVARIATE_RATIONAL_FUNCTION_APPROXIMATIONS_FOR_BL_AND_BU_OVER_BMAX )
    const double b_u = b_u_over_b_max(s_c) * b_max;
#else
    const double b_u = normalised_black(thetax, s_u);
#endif
    if (ibeta <= b_u) { // UPPER MIDDLE:  s_c <= s <= s_u
#if !defined( DO_NOT_SIMPLIFY_SU )
      const double inv_v_c = SQRT_TWO_PI / b_max;  // v_c = b_max * (1 / SQRT_TWO_PI)
#endif
      const double inv_v_u = inv_normalised_vega(thetax, s_u), r_um = convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(b_c, b_u, s_c, s_u, inv_v_c, inv_v_u, 0.0, false);
      s = rational_cubic_interpolation(ibeta, b_c, b_u, s_c, s_u, inv_v_c, inv_v_u, r_um);
      assert(s > 0);
#if defined (ENABLE_CHANGING_THE_MAXIMUM_ITERATION_COUNT) || defined( SAFEGUARD_STEP_AGAINST_BRACKETS )
      s_left = s_c;
      s_right = s_u;
#endif
    } else {       // HIGHEST BRANCH:  s_u < s  and  ibeta > b_max/2
      ...
      if (d2_f_upper_map_h_d_beta2 > -SQRT_DBL_MAX && d2_f_upper_map_h_d_beta2 < SQRT_DBL_MAX) {
        ...
        f = rational_cubic_interpolation(ibeta, b_u, b_max, f_upper_map_h, 0., d_f_upper_map_h_d_beta, -0.5, r_uu);
      }
      if (f <= 0) {
        const double h = b_max - b_u, t = (ibeta - b_u) / h;
        f = (f_upper_map_h * (1 - t) + 0.5 * h * t) * (1 - t); // We switch to quadratic interpolation using f(b_u), f(b_max)eequiv0, and f'(b_max)eequiv-1/2 to specify the quadratic.
      }
      ...
      s_left = s_u;
#endif
      if (ibeta > 0.5 * b_max) { // Else we better drop through and let the objective function be g(s) = b(thetax,s)-ibeta. 
        //
        ...
        //     s_n+1  =  s_n  +  ν · ( 1 + ν·h₂/2 ) / ( 1 + ν·( h₂ + ν·h₃/6 ) ).
        //
        const double ibeta_bar = b_max - ibeta;
        for (; iterations<N && fabs(ds)>DBL_EPSILON * s; ++iterations) {
#if defined (ENABLE_CHANGING_THE_MAXIMUM_ITERATION_COUNT)
          if (ds * ds_previous < 0)
            ++direction_reversal_count;
          ...
          if (b_bar < ibeta_bar && s < s_right) s_right = s; else if (b_bar > ibeta_bar && s > s_left) s_left = s; // Tighten the bracket if applicable.
...
#endif
          {
            ...
            // 
            //    b̄(θx,s)            =   ½ · [ erfc((s/2+θx/s)/√2)·bₘₐₓ + erfc((s/2-θx/s)/√2)/bₘₐₓ ]   (2)     |     using  Φ(z) = erfc(-z/√2)/2
            // 
            ...
            //               =   sc + [ exp(θx/2)·Φ(-θx/sc-sc/2)  +  exp(-θx/2)·Φ(θx/sc-sc/2) ] · exp(½·((θx/sc)²+(sc/2)²) · √(2π)               |   θx = - √(|x|·|x|), θx/sc = - √(|x|/2), sc/2 = +√(|x|/2), x/sc+sc/2 = 0, θx/sc-sc/2 = -sc
            //               =   √(2|x|) + [ ½·exp(θx/2)  +  exp(-θx/2)·Φ(-√(2|x|)) ] · exp(½·|x|) · √(2π)
            ...
            // 
            //    b̄(θx,s)   =   ½ · ( erfcx((s/2+θx/s)/√2)·exp(-½((θx/s)²+θx+(s/2)²))·exp(θx/2) + erfcx((s/2-θx/s)/√2)·exp(-½((θx/s)²-θx+(s/2)²))·exp(-θx/2) )
            //
            //              =   ½ · ( erfcx((s/2+θx/s)/√2) + erfcx((s/2-θx/s)/√2) ) · exp(-½((θx/s)²+(s/2)²))
            //
            //              =   √(π/2) · [ erfcx((s/2+θx/s)/√2) + erfcx((s/2-θx/s)/√2) ] · ∂b(θx,s)/∂s
            //
            // Ergo, ∂b(θx,s)/∂s / b̄(θx,s)  =  √(2/π) / ( erfcx((s/2+θx/s)/√2) + erfcx((s/2-θx/s)/√2) )
            //
            const double g = log(ibeta_bar / b_bar), xsuperscripttwo_over_ssuperscriptthree = (h * h) / s, b_h_2 = xsuperscripttwo_over_ssuperscriptthree - s / 4, c = 3 * (xsuperscripttwo_over_ssuperscriptthree / s) /* = (h/s)superscripttwo */, b_h_3 = b_h_2 * b_h_2 - c - 0.25;
...
#endif
    assert(s > 0);
    const double b = normalised_black(thetax, s), inv_bp = inv_normalised_vega(thetax, s), nu = (ibeta - b) * inv_bp, h = thetax / s, xsuperscripttwo_over_ssuperscriptthree = (h * h) / s, h_2 = xsuperscripttwo_over_ssuperscriptthree - s * 0.25, h_3 = h_2 * h_2 - 3 * (xsuperscripttwo_over_ssuperscriptthree / s) /* = (h/s)superscripttwo */ - 0.25;
#if defined (ENABLE_CHANGING_THE_MAXIMUM_ITERATION_COUNT)
    if (b > ibeta && s < s_right) s_right = s; else if (b<ibeta && s>s_left) s_left = s; // Tighten the bracket if applicable.
#endif
    ds = nu * householder_factor(nu, h_2, h_3);
...
double NormalisedImpliedBlackVolatility(double ibeta, double x, double theta /* q=+/-1 */) {
  // Map in-the-money to out-of-the-money
  return lets_be_rational(ibeta - normalised_intrinsic(theta < 0 ? -x : x), -fabs(x), IMPLIED_VOLATILITY_MAXIMUM_ITERATIONS);
}
...
double NormalisedImpliedBlackVolatilityForExcel(double ibeta, double x, double theta /* theta=+/-1 */, const XLOper & xlN) {
  // Map in-the-money to out-of-the-money
  return lets_be_rational(ibeta - normalised_intrinsic(theta < 0 ? -x : x), -fabs(x), xlN.isUndefined() ? IMPLIED_VOLATILITY_MAXIMUM_ITERATIONS : xlN.to_int());
}
...
DECLARE_XL_FUNCTION(Vega, { "","." }, "BBBBB$", "F,K,sigma,T", "The Black [1976] option value sensitivity partialBlack(F,K,sigma,T)/partialsigma.", { "the Forward.", "the Strike.", "the volatility.", "the time to expiry." });

DECLARE_XL_FUNCTION(NormalisedVega, { "","." }, "BBB$", "x,s", "The normalised Black option value sensitivity partialb(x,s)/partials.", { "x=ln(F/K)", "s=sigma*sqrtT." });

DECLARE_XL_FUNCTION(Volga, { "","." }, "BBBBB$", "F,K,sigma,T", "The Black [1976] option value sensitivity partialsuperscripttwoBlack(F,K,sigma,T)/partialsigmasuperscripttwo.", { "the Forward.", "the Strike.", "the volatility.", "the time to expiry." });

DECLARE_XL_FUNCTION(NormalisedVolga, { "","." }, "BBB$", "x,s", "The normalised Black option value sensitivity partialsuperscripttwob(x,s)/partialssuperscripttwo.", { "x=ln(F/K)", "s=sigma*sqrtT." });
...
DECLARE_XL_FUNCTION(ComplementaryNormalisedBlack, { "." }, "BBB$", "x,s",
                    "The complementary normalised Black option value hatb(x,s) = b_max(x,theta) - b(x,s,theta) = exp(12x)*Phi(-x/s-s/2) + exp(-12x)*Phi(x/s-s/2) with x=ln(F/K) and s=sigma*sqrt(T), where b_max = exp(theta*x/2). The dependency on theta cancels out.",
...
double BlOverBmax(double x) { return b_l_over_b_max(sqrt(2 * fabs(x))); }
DECLARE_XL_FUNCTION(BlOverBmax, { "." }, "BB$", "|x|", "bl(x)/bmax(x).", { "x=ln(F/K)." });
double BuOverBmax(double x) { return b_u_over_b_max(sqrt(2 * fabs(x))); }
...
double erfinv(double);
double Erf(double x) { return erf_cody(x); }
double ErfInv(double e) { return erfinv(e); }
DECLARE_XL_FUNCTION(Erf, { "." }, "BB$", "x", "returns erf(x) = int_0x exp(-zsuperscripttwo) dz * 2/sqrtpi.", { "x." });
DECLARE_XL_FUNCTION(ErfInv, { "." }, "BB$", "e", "returns the inverse of erf(x) = int_0x exp(-zsuperscripttwo) dz * 2/sqrtpi, i.e., finds x such that e = erf(x).", { "e." });
#endif
