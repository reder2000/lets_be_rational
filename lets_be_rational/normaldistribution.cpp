...
namespace {
  ...
  const double const1 = 0.180625; // = split1superscripttwo
...
double erfinv(double e) {
  // Φ(x) = erfc(-x/√2)/2 = (1+erf(x/√2))/2 = erf(x/√2)/2 + ½.
  // erf(z) = 2 · ( Φ(√2·z) - ½ ).
  // Hence, if 𝑒 = erfc(z), and y is the solution to Φ(y) - ½ = 𝑒/2, then z = y/√2.
...
namespace {
  // uₘₐₓ = Φ(1) - ½ = 0.3413447460685429
  const double u_max = 0.3413447460685429;
}

// Inverse of Φ(x)-½, i.e., for a given u ∈ [-uₘₐₓ,uₘₐₓ] find x such that u = Φ(x)-½.
inline double inverse_norm_cdfm12_for_midrange_probabilities(double u) {
  assert(!(fabs(u) > u_max)); // This formulation permits NaN to pass through below. The result will be NaN as it should.
  // innermost branch: x ∈ [-1,1], since u = Φ(x)-½ ⟹ u ∈ [-uₘₐₓ,uₘₐₓ] with uₘₐₓ=Φ(1)-½=0.3413447460685429, and p ∈ [0.15866,0.84134].
    // Define f(x) := √(2π)·(Φ(x)-½). It is an odd function: f(-x) = -f(x). Its Taylor expansion is f(x) = x - x³/6 + x⁵/40 - O(x⁷).
    ...
    // In the end, we transform to h(s) := √(2π)·(1+2π·(uₘₐₓ²-s)·g(2π·(uₘₐₓ²-s)))   with   s = uₘₐₓ² - u²  where uₘₐₓ = f(xₘₐₓ)/√(2π) = (Φ(xₘₐₓ)-½).  Note that this makes h(s) a (6,5) rational function.
    // Accuracy better than 9.8E-17 in perfect arithmetic within this branch.
  const double s = u_max * u_max - u * u;
...
double inverse_norm_cdf(double p) {
  ...
  if (fabs(u) < u_max)
      return inverse_norm_cdfm12_for_midrange_probabilities(u);
...
double erfinv(double e) {
  ...
  // Hence, if 𝑒 = erfc(z), and y is the solution to Φ(y) - ½ = 𝑒/2, then z = y/√2.
  if (fabs(e) < (2 * u_max))
    return inverse_norm_cdfm12_for_midrange_probabilities(0.5 * e) * (1 / SQRT_TWO);
